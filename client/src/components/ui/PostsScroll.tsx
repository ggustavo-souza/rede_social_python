            ...
            <>
                {posts.map((post: Post) => (
                    <div key={post.id} className="flex flex-col gap-2 border border-gray-300 rounded p-4 mb-4 text-center items-center">
                        {screen === "profile" && (
                            <button className="border border-gray-300 rounded px-3 py-2 animationBotao cursor-pointer hover:bg-(--color-secondary) text-sm self-end hover:text-(--color-primary) text-gray-500"><i className="bi bi-trash text-xl"></i></button>
                        )}
                ...
                {modal &&
                    <PostEditModal
                        field={field}
                        currentValue={posts.find(p => p.id === selectedPostId)?.titulo || ""}
